...
public class V7Loader extends V7LoaderBase {
            ...
            while (i < cmdEnd) {
                ...
                if (hasColumn(cells, CN_CMDVERIF_STAGE)) {
                    ...
                    if (type == CommandVerifier.Type.CONTAINER) {
                        ...
                        algo = makeAlgoVerifier(cmd, algo);
    ...
    private CustomAlgorithm makeAlgoVerifier(MetaCommand cmd, CustomAlgorithm algo0) {
        ...
        for (int i = 0; i < inputList.size(); i++) {
            ...
            if(qn!=null && qn.startsWith(YAMCS_CMDARG_SPACESYSTEM_NAME)) {
                String argName = qn.substring(YAMCS_CMDARG_SPACESYSTEM_NAME.length() + 1);
                Argument arg = cmd.getArgument(argName, true);
                if (arg == null) {
                    throw new SpreadsheetLoadException(ctx, "Invalid argument name '" + argName + "' used in algorithm "
                            + algo0.getName() + " which is used as verifier for command " + cmd.getName()
                            + ". The command has no such argument (direct or inherited)");
                }
                ArgumentInstanceRef argRef = new ArgumentInstanceRef(arg);
                argRef.setUseCalibratedValue(pref.useCalibratedValue());
                argRef.setMemberPath(pref.getMemberPath());

                InputParameter inputPara1 = new InputParameter(argRef, inputPara0.getDefinedInputName());
